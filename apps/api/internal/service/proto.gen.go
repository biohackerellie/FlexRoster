// api v0.0.1 dc30a6b55936821c9a031ff940859c740170959f
// --
...
func WebRPCSchemaHash() string {
	return "dc30a6b55936821c9a031ff940859c740170959f"
}
...
type RequestService interface {
	...
	NewRequest(ctx context.Context, studentName string, studentID string, requestStatus RequestStatus, dateRequested time.Time, newTeacher string, newTeacherName string, currentTeacher string, currentTeacherName string) error
...
func (s *requestServiceServer) serveNewRequestJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	reqPayload := struct {
		...
		Arg2 RequestStatus `json:"requestStatus"`
		Arg3 time.Time     `json:"dateRequested"`
		Arg4 string        `json:"newTeacher"`
		Arg5 string        `json:"newTeacherName"`
		Arg6 string        `json:"currentTeacher"`
		Arg7 string        `json:"currentTeacherName"`
	}{}
	if err := json.Unmarshal(reqBody, &reqPayload); err != nil {
		s.sendErrorJSON(w, r, ErrWebrpcBadRequest.WithCause(fmt.Errorf("failed to unmarshal request data: %w", err)))
		return
	...
	// Call service method implementation.
	err = s.RequestService.NewRequest(ctx, reqPayload.Arg0, reqPayload.Arg1, reqPayload.Arg2, reqPayload.Arg3, reqPayload.Arg4, reqPayload.Arg5, reqPayload.Arg6, reqPayload.Arg7)
	if err != nil {
		rpcErr, ok := err.(WebRPCError)
		if !ok {
			rpcErr = ErrWebrpcEndpoint.WithCause(err)
...
func (c *requestServiceClient) NewRequest(ctx context.Context, studentName string, studentID string, requestStatus RequestStatus, dateRequested time.Time, newTeacher string, newTeacherName string, currentTeacher string, currentTeacherName string) error {
	in := struct {
		Arg0 string        `json:"studentName"`
		Arg1 string        `json:"studentID"`
		Arg2 RequestStatus `json:"requestStatus"`
		...
		Arg6 string        `json:"currentTeacher"`
		Arg7 string        `json:"currentTeacherName"`
	}{studentName, studentID, requestStatus, dateRequested, newTeacher, newTeacherName, currentTeacher, currentTeacherName}
